    ...
    public abstract class SecurePage : Page {
        ...
        protected override void OnInit(EventArgs e) {
            ...
            if (CurrentRoleId < RequiredRoleId) {
                try {
                    int? uid = Session["UserId"] != null ? (int?)Session["UserId"] : null;
                    string email = Session["Email"]?.ToString() ?? "";
                    new SecurityLogService().LogEvent(uid, email, "UnauthorizedAccess",
                        $"Página: {Request.RawUrl} | Rol actual: {CurrentRoleId}, requerido: {RequiredRoleId}");
                } catch { }
        ...
        /// Valida sesión y rol. La protección CSRF viene de SameSite=Strict en las cookies.
        /// </summary>
        public static void CheckAccess(int requiredRoleId) {
            ...
            if (roleId < requiredRoleId) {
                try {
                    int? uid = session["UserId"] != null ? (int?)(int)session["UserId"] : null;
                    string email = session["Email"]?.ToString() ?? "";
                    new SecurityLogService().LogEvent(uid, email, "UnauthorizedAccess",
                        $"WebMethod bloqueado | Rol actual: {roleId}, requerido: {requiredRoleId}");
                } catch { }
