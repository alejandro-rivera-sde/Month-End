...
function renderSmtp(smtp) {
    ...
    const isRelay = smtp.authMode === 'relay';

    const authBadge = isRelay
        ? `<span class="es-smtp-badge es-smtp-badge-relay">${esT('email.smtp_auth_relay')}</span>`
        : `<span class="es-smtp-badge es-smtp-badge-creds">${esT('email.smtp_auth_creds')}</span>`;
    ...
    const fields = [
        ...
        { label: esT('email.smtp_ssl'),  value: smtp.ssl === 'true' ? 'TLS/SSL' : 'No', icon: 'lock' },
        {
            label: esT('email.smtp_auth'), icon: 'key',
            badge: authBadge,
            value: isRelay ? '' : smtp.user
        },
    ];
    ...
    grid.innerHTML = fields.map(f => `
        <div class="es-smtp-field">
            ...
            <div>
                ...
                <div class="es-smtp-value">
                    ${f.badge || ''}
                    ${f.value ? escH(f.value) : (!f.badge ? '—' : '')}
                </div>
            </div>
        </div>`).join('');
}
