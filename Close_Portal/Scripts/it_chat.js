...
(function () {
    ...
    function markMessageFailed(msgDiv) {
        ...
        if (bubble) {
            ...
            errEl.textContent = window.I18n ? window.I18n.t('chat.send_failed') : 'No enviado — intenta de nuevo';
    ...
    function renderCaseList(cases) {
        ...
        list.innerHTML = '';
        if (cases.length === 0) {
            list.innerHTML = '<div class="chat-empty-list" data-translate-key="chat.no_cases">' + (window.I18n ? window.I18n.t('chat.no_cases') : 'Sin casos abiertos en este cierre') + '</div>';
            return;
        }
    ...
    function showEmptyState() {
        ...
        c.innerHTML = [
            ...
            '<p data-translate-key="chat.empty_messages">' + (window.I18n ? window.I18n.t('chat.empty_messages') : 'Sin mensajes aún. ¡Escribe el primero!') + '</p>',
            '</div>'
        ].join('');
    }
    ...
    function showErrorState() {
        var c = document.getElementById('chatMessages');
        if (!c) return;
        c.innerHTML = '<div class="chat-error"><span class="material-icons">error_outline</span><p data-translate-key="chat.error_loading">' + (window.I18n ? window.I18n.t('chat.error_loading') : 'Error al cargar mensajes.') + '</p></div>';
    }
