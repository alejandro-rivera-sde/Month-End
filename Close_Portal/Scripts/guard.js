...
// ─── Caché de datos para re-render sin AJAX ─────────────────
let gd_cachedActiveLocs = null;   // resultado de GetAllActiveLocations
let gd_cachedGuardLocs  = null;   // resultado de GetActiveGuardLocations (modo confirmado)
let gd_cachedSavedIds   = null;   // Set<locationId> guardados (modo borrador)
let gd_cachedHistory    = null;   // historial de guardias
...
// Re-renderiza las locaciones desde caché al cambiar idioma (sin AJAX)
function reRenderLocations() {
    const gridEl = document.getElementById('gdLocGrid');
    if (!gridEl || !gd_guard) return;

    if (gd_guard.isDraft && gd_cachedActiveLocs && gd_cachedSavedIds) {
        _doRenderLocationPicker(gridEl, gd_cachedActiveLocs, gd_cachedSavedIds);
    } else if (gd_guard.isConfirmed && gd_cachedGuardLocs) {
        gridEl.innerHTML = gd_cachedGuardLocs.map(loc => renderLocationCard(loc)).join('');
    }
}
...
function renderLocationPicker(gridEl, allLocations) {
    gdCall('GetActiveGuardLocations', {}, (savedResp) => {
        ...
        gd_cachedSavedIds = savedIds;
        _doRenderLocationPicker(gridEl, allLocations, savedIds);
    });
}

function _doRenderLocationPicker(gridEl, allLocations, savedIds) {
        ...
        const missingLabel = (loc) => {
            if (!loc.hasAdmin && !loc.hasRegular) return t('gd.loc.missing_both', 'Admin · Regular');
            if (!loc.hasAdmin) return t('gd.loc.missing_admin', 'Admin');
            return t('gd.loc.missing_regular', 'Regular');
        };
            ...
            <div class="gd-loc-unattended-group">
                <div class="gd-loc-unattended-header">
                    ...
                    <div class="gd-loc-legend">
                        ...
                        <span class="gd-loc-legend-label">${t('gd.loc.missing_both', 'Sin Admin y Regular')}</span>
                        <span class="gd-loc-legend-dot gd-loc-missing-admin"></span>
                        <span class="gd-loc-legend-label">${t('gd.loc.missing_admin', 'Sin Admin')}</span>
                        <span class="gd-loc-legend-dot gd-loc-missing-regular"></span>
                        <span class="gd-loc-legend-label">${t('gd.loc.missing_regular', 'Sin Regular')}</span>
                    </div>
                </div>
...
function loadHistory() {
    gdCall('GetGuardHistory', {}, (resp) => {
        ...
        gd_cachedHistory = resp.data || [];
        _doRenderHistory(gd_cachedHistory);
    });
}

function reRenderHistory() {
    if (gd_cachedHistory) _doRenderHistory(gd_cachedHistory);
}

function _doRenderHistory(items) {
