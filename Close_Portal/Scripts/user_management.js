...
function buildWmsChecklist(wmsList) {
    ...
    if (!wmsList || wmsList.length === 0) {
        container.innerHTML =
            `<div style="color:var(--text-muted);font-size:13px;padding:8px">${getTranslation('um.wms.empty')}</div>`;
        return;
    }
...
function buildLocationChecklist(locationList) {
    ...
    if (!locationList || locationList.length === 0) {
        container.innerHTML =
            `<div style="color:var(--text-muted);font-size:13px;padding:8px">${getTranslation('um.loc.empty')}</div>`;
        return;
    }
...
function buildWmsChecklistInto(containerId, wmsList, assignedIds) {
    ...
    if (!wmsList || wmsList.length === 0) {
        container.innerHTML = `<div style="color:var(--text-muted);font-size:13px;padding:8px">${getTranslation('um.wms.empty')}</div>`;
        return;
    }
