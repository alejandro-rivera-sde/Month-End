    ...
    public partial class EmailServicePage : SecurePage {
        ...
        public static object GetEmailConfig() {
            try {
                ...
                bool hasCredentials = !string.IsNullOrWhiteSpace(appCfg["Smtp_User"])
                                   && !string.IsNullOrWhiteSpace(appCfg["Smtp_Password"]);
                var smtp = new {
                    host     = appCfg["Smtp_Host"]      ?? "—",
                    port     = appCfg["Smtp_Port"]      ?? "587",
                    from     = appCfg["Smtp_From"]      ?? "—",
                    ssl      = appCfg["Smtp_EnableSsl"] ?? "true",
                    authMode = hasCredentials ? "credentials" : "relay",
                    user     = hasCredentials ? appCfg["Smtp_User"] : ""
                };
                ...
                AppLogger.Error("EmailService.GetEmailConfig", ex);
        ...
        public static object SetBulkAlerts(bool enabled) {
            try {
                ...
                var allKeys = GetAlertMeta().Select(m => m.Key).ToArray();
                var da = new EmailDataAccess();
                foreach (var key in allKeys) {
                    da.SetAlertEnabled(key, enabled, userId);
        ...
        public static object SendTestEmail(string alertKey, string overrideRecipient) {
            try {
                ...
                AppLogger.Error("EmailService.SendTestEmail", ex);
                return new { success = false, message = ex.Message };
            }
        }
        ...
        private static void SendTestAlertByKey(string key, string performer) {
            switch (key) {
                ...
                case "UserRemoved":
                    ...
                    break;
                case "UserUpdated":
                    EmailService.NotifyUserUpdated("test@example.com", "Usuario de Prueba",
                        new List<string> { "Rol: Regular → Administrador", "Teléfonos: 1 número(s) → 2 número(s)" }, performer);
                    break;
                case "ClosureRequest":
                ...
                case "DefaultSpotReminder":
                    ...
                    break;
                case "LocationReverted":
                    EmailService.NotifyLocationReverted(0, "Bodega de Prueba", performer);
                    break;
            }
        }
        ...
        private static List<AlertMetaItem> GetAlertMeta() =>
            new List<AlertMetaItem> {
                ...
                new AlertMetaItem { Key="GuardCancelled",      Label="Guardia confirmada cancelada",       Icon="cancel",           ConfigurableRecipient=true,  DefaultGroupKey="Administradores",  FixedRecipientDesc=null },
                ...
                new AlertMetaItem { Key="LocationReverted",    Label="Locación revertida a operación",     Icon="undo",             ConfigurableRecipient=true,  DefaultGroupKey="Administradores", FixedRecipientDesc=null },
                // ── Usuarios ──────────────────────────────────────────────────
