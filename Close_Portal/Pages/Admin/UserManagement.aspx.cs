    ...
    public partial class UserManagement : SecurePage {
        ...
        private void LoadStats() {
            try {
                ...
                AppLogger.Error("UserManagement.LoadStats", ex);
            }
        }
        ...
        private void LoadLocationsFilter() {
            try {
                ...
                AppLogger.Error("UserManagement.LoadLocationsFilter", ex);
            }
        }
            ...
            try {
                using (SqlConnection conn = new SqlConnection(_connStr)) {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn)) {
                        ...
                        // Owners pueden ver y asignarse el rol Owner (al editar su propio perfil).
                        // Admins y roles inferiores solo ven hasta Administrador.
                        int callerRole = Session["RoleId"] != null ? (int)Session["RoleId"] : 0;
                        DataTable filtered;
                        if (callerRole < RoleLevel.Owner) {
                            dt.DefaultView.RowFilter = "Role_Id < 4";
                            filtered = dt.DefaultView.ToTable();
                ...
                AppLogger.Error("UserManagement.LoadRoles", ex);
            }
        }
        ...
        private void LoadDepartments() {
            try {
                ...
                AppLogger.Error("UserManagement.LoadDepartments", ex);
            }
        }
        ...
        private void LoadUsers() {
            try {
                ...
                AppLogger.Error("UserManagement.LoadUsers", ex);
            }
        }
            ...
            try {
                ...
                using (var conn = new SqlConnection(_connStr)) {
                    ...
                    using (var cmd = new SqlCommand(
                        ...
                        using (var r = cmd.ExecuteReader()) {
                            while (r.Read()) {
                                user.Phones.Add(new UserPhoneModel {
                ...
                AppLogger.Error("UserManagement.GetUserRowData", ex);
                return null;
            }
        }
        ...
        public static object GetUserDetail(int userId) {
            try {
                ...
                using (SqlConnection conn = new SqlConnection(_connStr)) {
                    ...
                    // ── 4. Teléfonos activos del usuario ─────────────────────
                    var phoneList = new List<object>();
                ...
                AppLogger.Error("UserManagement.GetUserDetail", ex);
        ...
        public static object GetAllWms() {
            try {
                ...
                AppLogger.Error("UserManagement.GetAllWms", ex);
                return new { Success = false, Message = ex.Message };
            }
        }
        ...
        public static object GetAllLocations() {
            try {
                ...
                AppLogger.Error("UserManagement.GetAllLocations", ex);
                return new { Success = false, Message = ex.Message };
            }
        }
        ...
        public static object GetAllDepartments() {
            try {
                ...
                AppLogger.Error("UserManagement.GetAllDepartments", ex);
                return new { Success = false, Message = ex.Message };
            }
        }

        // ============================================================
        // SNAPSHOT — estado previo del usuario para detección de cambios
        // ============================================================
        ...
        private static UserSnapshot GetUserSnapshot(int userId) {
            var snap = new UserSnapshot { Phones = new List<string>() };
            using (var conn = new SqlConnection(_connStr)) {
                conn.Open();
                using (var cmd = new SqlCommand(@"
                    SELECT u.Role_Id, r.Role_Name, u.Active, u.Locked,
                           u.First_Name, u.Last_Name, u.Department_Id, d.Department_Code
                    FROM   MonthEnd_Users u
                    INNER JOIN MonthEnd_Users_Roles r ON r.Role_Id = u.Role_Id
                    LEFT  JOIN MonthEnd_Departments d ON d.Department_Id = u.Department_Id
                    WHERE  u.User_Id = @UserId", conn)) {
                    cmd.Parameters.AddWithValue("@UserId", userId);
                    using (var r = cmd.ExecuteReader()) {
                        if (r.Read()) {
                            snap.RoleId        = (int)r["Role_Id"];
                            snap.RoleName      = r["Role_Name"].ToString();
                            snap.Active        = (bool)r["Active"];
                            snap.Locked        = (bool)r["Locked"];
                            snap.FirstName     = r["First_Name"] as string;
                            snap.LastName      = r["Last_Name"]  as string;
                            snap.DepartmentId  = r["Department_Id"]  as int?;
                            snap.DepartmentCode= r["Department_Code"] as string;
                        }
                    }
                }
                using (var cmd = new SqlCommand(
                    "SELECT Phone FROM MonthEnd_User_Phones WHERE User_Id = @UserId AND Active = 1 ORDER BY Phone", conn)) {
                    cmd.Parameters.AddWithValue("@UserId", userId);
                    using (var r = cmd.ExecuteReader())
                        while (r.Read()) snap.Phones.Add(r["Phone"].ToString());
                }
            }
            return snap;
        }
        ...
        //   4. DELETE + INSERT MonthEnd_User_Phones
        // ============================================================
        [WebMethod(EnableSession = true)]
        public static object SaveUserChanges(
            ...
            string[] phones, string[] phoneExtensions,
            ...
            try {
                ...
                // Solo Owners pueden asignar el rol Owner (previene escalada de privilegios)
                if (roleId >= RoleLevel.Owner && currentRoleId < RoleLevel.Owner)
                    return new { Success = false, Message = "No tienes permisos para asignar ese rol." };

                // Capturar estado previo para detección de cambios reales
                var snap = GetUserSnapshot(userId);
                ...
                object lockReason = locked
                    ? (object)$"Bloqueado manualmente por: {performedBy}"
                    : DBNull.Value;
                ...
                using (SqlConnection conn = new SqlConnection(_connStr)) {
                    ...
                    using (SqlTransaction tx = conn.BeginTransaction()) {
                        try {
                            ...
                            cmdUser.Parameters.AddWithValue("@LockReason", lockReason);
                            ...
                            // 4. Reemplazar teléfonos: eliminar los actuales e insertar los nuevos
                            using (SqlCommand cmd = new SqlCommand(
                                "DELETE FROM MonthEnd_User_Phones WHERE User_Id = @UserId", conn, tx)) {
                                cmd.Parameters.AddWithValue("@UserId", userId);
                                cmd.ExecuteNonQuery();
                            }
                            if (phones != null) {
                                for (int i = 0; i < phones.Length; i++) {
                                    ...
                                    string ext = (phoneExtensions != null && i < phoneExtensions.Length)
                                                 ? phoneExtensions[i]?.Trim() : null;
                                    using (SqlCommand cmd = new SqlCommand(
                            ...
                            tx.Rollback();
                            AppLogger.Error("UserManagement.SaveUserChanges tx", ex);
                            return new { Success = false, Message = "Error al guardar cambios" };
                        }
                    }
                }

                // Detectar cambios reales para el correo de modificación
                var changes = new List<string>();
                var secLog  = new SecurityLogService();

                if (snap.RoleId != roleId) {
                    changes.Add($"Rol: {snap.RoleName} → {newRoleName}");
                    secLog.LogEvent(userId, targetEmail, "RoleChanged",
                        $"{snap.RoleName} → {newRoleName} | por {performedBy}");
                }

                if (snap.Active != active) {
                    changes.Add($"Estado: {(snap.Active ? "Activo" : "Inactivo")} → {(active ? "Activo" : "Inactivo")}");
                    secLog.LogEvent(userId, targetEmail, active ? "AccountActivated" : "AccountDeactivated",
                        $"Por {performedBy}");
                }

                string oldName = $"{snap.FirstName} {snap.LastName}".Trim();
                string newNameStr = $"{firstName?.Trim()} {lastName?.Trim()}".Trim();
                if (!string.Equals(oldName, newNameStr, StringComparison.OrdinalIgnoreCase))
                    changes.Add($"Nombre: {(string.IsNullOrEmpty(oldName) ? "—" : oldName)} → {(string.IsNullOrEmpty(newNameStr) ? "—" : newNameStr)}");

                int? newDeptId = departmentId > 0 ? (int?)departmentId : null;
                if (snap.DepartmentId != newDeptId) {
                    string newDeptCode = "—";
                    if (newDeptId.HasValue) {
                        using (var c = new SqlConnection(_connStr))
                        using (var cmd = new SqlCommand("SELECT Department_Code FROM MonthEnd_Departments WHERE Department_Id = @Id", c)) {
                            cmd.Parameters.AddWithValue("@Id", newDeptId.Value);
                            c.Open();
                            newDeptCode = cmd.ExecuteScalar()?.ToString() ?? "—";
                        }
                    }
                    changes.Add($"Departamento: {snap.DepartmentCode ?? "—"} → {newDeptCode}");
                }

                var newPhonesSorted = (phones ?? new string[0])
                    .Where(p => !string.IsNullOrWhiteSpace(p))
                    .Select(p => p.Trim())
                    .OrderBy(p => p)
                    .ToList();
                if (!snap.Phones.SequenceEqual(newPhonesSorted))
                    changes.Add($"Teléfonos: {snap.Phones.Count} número(s) → {newPhonesSorted.Count} número(s)");

                if (!string.IsNullOrEmpty(newPassword)) {
                    changes.Add("Contraseña actualizada");
                    secLog.LogEvent(userId, targetEmail, "PasswordChangedByAdmin",
                        $"Por {performedBy}");
                }

                if (changes.Count == 0)
                    changes.Add("Sin cambios detectados en los campos principales");
                ...
                if (locked && !snap.Locked) {
                    System.Threading.Tasks.Task.Run(() =>
                        EmailService.NotifyUserBlocked(targetEmail, targetFullName, performedBy));
                } else if (!locked && snap.Locked) {
                    System.Threading.Tasks.Task.Run(() =>
                        EmailService.NotifyUserUnblocked(targetEmail, targetFullName, performedBy));
                }

                if (locked != snap.Locked)
                    secLog.LogEvent(userId, targetEmail, locked ? "AccountLocked" : "AccountUnlocked",
                        locked ? $"Bloqueado manualmente por: {performedBy}" : $"Desbloqueado por: {performedBy}");

                return new { Success = true, Message = "Cambios guardados correctamente", Row = GetUserRowData(userId) };

            } catch (Exception ex) {
                AppLogger.Error("UserManagement.SaveUserChanges", ex);
                return new { Success = false, Message = ex.Message };
            }
        }
        ...
        public static object CreateUser(
            ...
            try {
                ...
                using (var conn = new SqlConnection(_connStr)) {
                    ...
                    using (var tx = conn.BeginTransaction()) {
                        try {
                            ...
                            tx.Rollback();
                            AppLogger.Error("UserManagement.CreateUser tx", ex);
                ...
                new SecurityLogService().LogEvent(newUserId, email, "UserCreated",
                    $"Rol: {roleName} | por {createdBy}");

                return new { Success = true, Message = "Usuario creado correctamente", UserId = newUserId };

            } catch (Exception ex) {
                AppLogger.Error("UserManagement.CreateUser", ex);
                return new { Success = false, Message = ex.Message };
            }
        }
